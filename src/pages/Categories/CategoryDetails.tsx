    ...
    { 
      ...
      render: (sub: SubCategory) => (
        <div className="flex justify-end items-center gap-4">
          <button onClick={() => setEditingSub(sub)} className="p-2  hover:text-gray-300 text-[#f97316] transition-colors cursor-pointer">
            <Edit3 size={18} />
          </button>
          <button onClick={(e) => handleDeleteSubCategory(e, sub.id)} className="p-2 hover:text-gray-300 text-red-500 transition-colors cursor-pointer">
            <Trash2 size={18} />
          </button>
        </div>
      )
    ...
    <div className="p-8 bg-[#fafafa] min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[32px] font-black text-[#1a1a1a]">
              {currentCategory ? t("", { defaultValue: currentCategory.name }) : t('loading')}
            </h1>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setIsEditCatOpen(true)} className="flex items-center gap-2 px-5 py-4 border-2 border-orange-100 text-[#f97316] font-bold rounded-2xl hover:bg-orange-50 transition-all cursor-pointer">
            <Edit3 size={20} /> <span className="hidden sm:inline">{t('update')}</span>
          </button>
          <button onClick={handleDeleteCategory} className="flex items-center gap-2 px-5 py-4 border-2 border-red-100 text-red-500 font-bold rounded-2xl hover:bg-red-50 transition-all cursor-pointer">
            <Trash2 size={20} /> <span className="hidden sm:inline">{t('delete')}</span>
          </button>
          <button onClick={() => navigate(`/add-subcategory/${categoryId}`)} className="flex items-center gap-2 px-7 py-4 bg-[#f97316] text-white font-bold rounded-2xl shadow-lg hover:bg-[#ea580c] transition-all cursor-pointer">
