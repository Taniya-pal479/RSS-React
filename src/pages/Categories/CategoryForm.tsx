...
const CategoryForm = ({ mode }: { mode: "category" | "subcategory" }) => {
  ...
  return (
    <div className="max-w-200 mx-auto bg-white rounded-3xl border border-gray-100 p-10 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-[13px] font-bold text-gray-400 mb-2.5 ml-1">
                {mode === "category" ? t("CatName") : t("SubcatName")}{" "}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-5 py-4 bg-[#f9fafb] border border-gray-200 rounded-[14px] text-[16px] outline-none focus:border-[#f97316] transition-all min-h-35 resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={isLoading}
            className="flex items-center gap-3 px-10 py-4 bg-[#f97316] text-white font-bold rounded-[14px] hover:bg-[#ea580c] transition-all shadow-[0_10px_20px_-5px_rgba(249,115,22,0.4)] disabled:opacity-50 active:scale-95 cursor-pointer"
          >
            {isLoading ? (
