...
const ContentTypeForm: React.FC = () => {
  ...
  const { data: response } = useGetContentTypesQuery(
    { categoryId: urlCatId, lang: globalLang, take: 10, skip: 0 },
    { skip: !id || !urlCatId },
  );
  const contentTypesList = response?.data || [];
  ...
  useEffect(() => {
    if (itemToEdit && selectedCatId === "") {
      ...
      if (itemToEdit.translations) {
        itemToEdit.translations.forEach((trans: Translation) => {
